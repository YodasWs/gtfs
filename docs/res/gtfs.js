		...
		loadedFiles: {
			...
			set(value) {
				...
				if ([
					...
					const mappedTrips = new Map();

					// Record the longest trip for each direction of each route
					Object.values(this.trips).forEach((trip) => {
						if (!mappedTrips.has(trip.route_id) || !(mappedTrips.get(trip.route_id) instanceof Map)) {
							// Map of each route
							mappedTrips.set(trip.route_id, new Map());
						}
						const route = mappedTrips.get(trip.route_id);
						if (!trip.direction_id) {
							// Default direction
							trip.direction_id = 0;
						}
						if (!route.has(trip.direction_id) || !Array.isArray(route.get(trip.direction_id))) {
							// Array of stops in this direction
							route.set(trip.direction_id, trip.stops);
						}
						if (Array.isArray(trip.stops) && trip.stops.length > route.get(trip.direction_id).length) {
							route.set(trip.direction_id, trip.stops);
						}
					});

					// Link to a shape
					mappedTrips.forEach((mapRoute, route_id) => {
						const objRoute = this.routes[route_id];
						if (typeof objRoute !== 'object' || objRoute === null) return;
						const routeHasShape = objRoute.shapes instanceof Set && objRoute.shapes.size > 0;
						mapRoute.forEach((stops, direction_id) => {
							if (!Array.isArray(stops)) return;
							if (route_id === '19') {
								console.log('Sam, route', route_id, 'direction', direction_id);
							}
							const path = [];
							// List stops in stop order
							stops.sort((a, b) => {
								return Number.parseFloat(a.stop_sequence) - Number.parseFloat(b.stop_sequence);
							}).forEach((stop, i, array) => {
								// Extend stop information
								array[i] = {
									...stop,
									...this.stops[stop.stop_id],
								};
								// If no route shape provided, we build our own
								if (!routeHasShape) {
									path.push({
					...
					postMessage(['listStops', mappedTrips]);
